...
import {ProductAttributeValueViewModel} from './product-attribute-value/product-attribute-value.viewmodel';
...
export class ProductAttributeService implements Resolve<any> {
    url = 'api/v1/products/product-attributes';
    ...
    searchValues(attributeId: string, keyword: string, isActive?: boolean, page?: number,
                 pageSize?: number): Observable<SearchResultViewModel<ProductAttributeValueViewModel>> {
        return this.http.get(`${this.url}/${attributeId}/values`, {
            params: new HttpParams()
                ...
                .set('page', page ? page.toString() : '1')
                .set('pageSize', pageSize ? pageSize.toString() : this.appConfig.PAGE_SIZE.toString())
        }) as Observable<SearchResultViewModel<ProductAttributeValueViewModel>>;
    }
    ...
    suggestions(keyword: string): Observable<SearchResultViewModel<NhSuggestion>> {
    ...
    suggestionValue(productAttributeId: string, keyword: string): Observable<SearchResultViewModel<NhSuggestion>> {
